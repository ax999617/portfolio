...
本地审计记录显示三类来源已找回，但它仍是未经来源分组的类别平铺归档；该机器本地记录不是公开数据集。本阶段继续采用保守策略：突出算法与可靠工程，不启动真实训练。完整分析见 [`ALGORITHM_REFACTOR_REPORT.md`](ALGORITHM_REFACTOR_REPORT.md)。
...
- 恢复设计：默认 `num_workers=0`，并在检查点中保存/恢复随机状态；现有测试验证恢复入口可继续执行，但不宣称与不间断训练逐位等价。若为吞吐量改用多进程加载，必须关闭确定性模式并记录取舍。
...
- `henan-complete-source`：机器本地只读的三类平铺归档。
- `henan-incomplete-backup`：机器本地只读的两类备份。
...
精确机器路径仅保留在算法报告末尾的折叠溯源记录中，不是可移植的运行参数；公开命令使用环境变量，其他机器应指向各自的数据副本。
